...
- (void)reloadImage
{
  ...
  if (source && self.frame.size.width > 0 && self.frame.size.height > 0) {
    ...
    __weak RNImageView *weakSelf = self;
    ...
    RCTImageLoaderCompletionBlock completionHandler = ^(NSError *error, UIImage *loadedImage) {
      ...
      [manager loadImageWithURL:source.request.URL
                                               options:0
                                             ...
                                             completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, SDImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
                                                  if(data)
                                                  {
                                                      UIImage *resultImage = RCTDecodeImageWithData(data, imageSize, imageScale, weakSelf.resizeMode);
                                                      completionHandler(error, resultImage);
                                                  }
                                                  else
                                                  {
...
- (void)reactSetFrame:(CGRect)frame
{
  ...
  if ([self shouldChangeImageSource]) {
    ...
    CGSize sourceize = (_imageSource ? _imageSource : _pendingImageSource).size;
    if (imageSize.width * imageScale == sourceize.width * _imageSource.scale &&
        imageSize.height * imageScale == sourceize.height * _imageSource.scale) {
      return;
    }
